...
// inherit methods and properties of Writable
utillib.inherits(MailParser, Writable);
 ...
 */
MailParser.prototype._write = function(chunk, encoding, done) {
    if (this.writeInternal(chunk, encoding)) {
 ...
 * @param {String} [encoding] The encoding to be used when "chunk" is a string
 */
MailParser.prototype._final = function(done) {
 ...
 */
MailParser.prototype.writeInternal = function(chunk, encoding) {
...
MailParser.prototype._process = function(finalPart) {
    ...
    if (finalPart) {
        // Clear remainder from memory since it will not be used
    ...
    if (!line.length) {
        ...
        if (this._currentNode.attachment) {
            ...
            if (this.options.streamAttachments) {
                ...
                attachment.stream.on('meta-data', (metadata) => {
                    this._currentNode.meta.checksum = metadata.checksum;
                    this._currentNode.meta.length = metadata.length;
                });
...
MailParser.prototype._processMimeTree = function() {
    ...
    if (typeof setImmediate == "function") {
        setImmediate(this.emit.bind(this, "email", returnValue));
    } else {
        process.nextTick(this.emit.bind(this, "email", returnValue));
    }
};
